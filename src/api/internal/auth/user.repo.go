...
func (r *UserRepo) CreateUser(user *models.UserModel) error {
	query := `INSERT INTO users (username, password, salt, created_at, created_by) VALUES (?, ?, ?, ?, ?)`
	_, err := r.DB.Exec(query, user.Username, user.Password, user.Salt, user.CreatedAt, user.CreatedBy)
	return err
}
